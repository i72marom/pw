...
/**
 * Clase que implementa la interfaz AnuncioDAO para hacer las consultas a la BBDD relacionadas con la clase Anuncio en MySQL.
 * @author Javi y Manu
 *
 */
public class MySQLAnuncioDAO implements AnuncioDAO{
	...
	/**
	 * Constructor de la clase MySQLAnuncioDAO
	 * @param conn
	 */
	...
	/**
	 * Función que inserta un anuncio en la BBDD
	 */
	...
	@Override
	/**
	 * Funcion que borra un anuncio de la BBDD
	 */
	public void borrar(Anuncio a) throws DAOException {
		...
		final String borrarIndividualizadoQuery = prop.getProperty("borrarIndividualizado");
		final String borrarFlashQuery = prop.getProperty("borrarFlash");
		final String borrarDestinatariosQuery = prop.getProperty("borrarDestinatarios");
		try {
			
			Anuncio anuncioABorrar = obtener(a.getId());
			
			System.out.println(anuncioABorrar.getTipo());
			...
			else if(anuncioABorrar.getTipo() == Tipo.individualizado)
			{
				stat = conn.prepareStatement(borrarIndividualizadoQuery);
				stat.setLong(1, a.getId());
				if(stat.executeUpdate() == 0)
				{
					throw new DAOException("Error en SQL.");
				}
				borrarDestinatarios(a.getId());
				
				
			}
			else if(anuncioABorrar.getTipo() == Tipo.flash)
			{
				System.out.println("aqui entra");
				stat = conn.prepareStatement(borrarFlashQuery);
				stat.setLong(1, a.getId());
				if(stat.executeUpdate() == 0)
				{
					throw new DAOException("Error en SQL.");
	...
	@Override
	/**
	 * Función que modifica un anuncio en la BBDD
	 */
	public void Modificar(Anuncio a) throws DAOException{
		...
		try {
			...
			stat.setDate(3, java.sql.Date.valueOf(lcdate));
	...
	@Override
	/**
	 * Función que obtiene un anuncio de la BBDD a partir de su ID
	 */
	...
	/**
	 * Funcion que obtiene un anuncio a través de la Id de su autor.
	 */
	...
	public void borrarDestinatarios(Long idAnuncio) throws DAOException
	{
		PreparedStatement stat = null;
		
		final String insertNuevoDestinatarioQuery = prop.getProperty("borrarDestinatarios");
	...
	public void modificarTags(Long idAnuncio, ArrayList<String> tags_) throws DAOException
	{
		PreparedStatement stat = null;
	...
	/**
	 * Función para actualizar las fechas de un anuncio flash.
	 */
